...
#        print(pygame.font.get_fonts()) # 使えるフォント名
    ...
    def __draw_vartical_lines(self):
        for xi in range(len(self.__ghostleg.Ghostleg)+1):
            ...
            start = (self.__CalcSize.StartPoint[0] + xi * self.__CalcSize.WidthInterval, self.__CalcSize.StartPoint[1])
            end = (self.__CalcSize.StartPoint[0] + xi * self.__CalcSize.WidthInterval, self.__screen.Size[1] - self.__CalcSize.FontPixcelSize * self.__CalcSize.GoalStrMaxLen)
...
#        print('font.size():', font.size())
        for i in range(len(g.Goals)):
            self.__screen.Screen.blit(font.render(g.Goals[i], False, self.__color), (self.__CalcSize.StartPoint[0] + i * self.__CalcSize.WidthInterval, self.__screen.Size[1] - self.__CalcSize.FontPixcelSize * self.__CalcSize.GoalStrMaxLen))
        ...
        for yi in range(len(self.__ghostleg.Ghostleg[0])):
            for xi in range(len(self.__ghostleg.Ghostleg)):
                if 1 == self.__ghostleg.Ghostleg[xi][yi]:
                    start = (self.__CalcSize.StartPoint[0] + xi * self.__CalcSize.WidthInterval, self.__CalcSize.StartPoint[1] + (yi+1) * self.__CalcSize.HeightInterval)
                    end = (self.__CalcSize.StartPoint[0] + (xi+1) * self.__CalcSize.WidthInterval, self.__CalcSize.StartPoint[1] + (yi+1) * self.__CalcSize.HeightInterval)
...
#                    self.__to_goal_pointlist.append([self.__CalcSize.StartPoint[0] + (now_line_index+1) * self.__CalcSize.WidthInterval, self.__CalcSize.StartPoint[1] + ((y+1) * self.__CalcSize.HeightInterval)])
#                    self.__to_goal_pointlist.append([self.__CalcSize.StartPoint[0] + (now_line_index+1) * self.__CalcSize.WidthInterval, self.__CalcSize.StartPoint[1] + ((y+1) * 24)])
                    now_line_index += 1
                else: # │
                    self.__set_pointlist_value(now_line_index, y+1)
                    self.__append_point(now_line_index, y+2)
#                    self.__to_goal_pointlist.append([self.__CalcSize.StartPoint[0] + now_line_index * self.__CalcSize.WidthInterval, self.__CalcSize.StartPoint[1] + (y+2) * self.__CalcSize.HeightInterval])
...
#            self.__to_goal_pointlist.append([self.__CalcSize.StartPoint[0] + now_line_index * self.__CalcSize.WidthInterval, self.__CalcSize.StartPoint[1] + (y * self.__CalcSize.HeightInterval)])
...
#            self.__to_goal_pointlist.append([20 + now_line_index * 40, 20 + (y * 24)])
    ...
    def __append_point(self, now_line_index, y):
        self.__to_goal_pointlist.append([
            self.__CalcSize.StartPoint[0] + now_line_index * self.__CalcSize.WidthInterval, 
            self.__CalcSize.StartPoint[1] + (y * self.__CalcSize.HeightInterval)])
